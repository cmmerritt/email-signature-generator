...
export default function Signature() {
  ...
  const quoteNoPunct = quote.replace(/[.,\/#!$%\^?&\*;:{}=\-_`~()]/g,"");    const quoteForToken = quoteNoPunct.replace(/\s{2,}/g," ");
  ...
  const copyToClipboard = () => {
    let textToCopy = "<div style=\"font-family: " + userFont + "; color: " + userColor + "\"><div>" + quote + "</div><br /><div>~" + randomAuthor + "</div><br /><img src=\"" + gifUrl + "\" /><br /><div>Generated by <a href=\"my-email-signature.netlify.app\">My Email Signature Generator</a></div>"
  ...
  for(let i = 0; i < quoteWords.length; i++) {
    ...
    if(tokens.includes(wordNoPunct)) {
      renderedQuote.push(<span key={i} value={quoteWords[i]} onClick={handleNewCategoryChoice} style={{ fontFamily: `${userFont}`, color: `${userColor}` }}>{quoteWords[i]} </span>);
    } else {
      renderedQuote.push(<span key={i} style={{ fontFamily: `${userFont}`, color: `${userColor}` }}>{quoteWords[i]} </span>);
    }
  };
  ...
  return (
    <Container>
    <main>
        <h1 style={{ fontFamily: "Arial" }}>Your New Email Signature</h1>
        {navigation.state !== "idle" ? <div>Loading...</div> : null}
        <span style={{ fontFamily: "Arial" }}><i>Click on a keyword in the quote to get a new gif based on that keyword.</i></span>
      <br />
      <br />
      ...
      <br />
        ...
        <div style={{ fontFamily: `${userFont}`, color: `${userColor}` }}>
            ~{randomAuthor}
        </div>
      ...
      <Button variant="outlined" onClick={ copyToClipboard }>Click to copy your HTML email signature</Button>
